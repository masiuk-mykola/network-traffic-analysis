...
| 0.3 | `feat(api): add query keys and a client error type` | One exported key factory per resource (`sensors`, `fields`, `search`, `searchResults`, `session`, `protocolSchema`) so nothing fetches the same thing under two keys, plus the `HttpError` the proxy returns. | **unit**: keys are stable and collision-free       |
